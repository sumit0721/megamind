  ...
  return (
    <div>
      ...
      >
        ...
        {/* Content — centered */}
        <motion.div
          className="relative z-10 max-w-container mx-auto px-6 flex flex-col items-center justify-center text-center"
          style={{ minHeight: '90vh', opacity: heroOpacity }}
        >
          <Reveal variant="fadeDown" delay={0.1}>
            <p className="text-accent font-semibold tracking-widest uppercase text-sm mb-6 inline-flex items-center gap-2">
          ...
          <Reveal variant="blurIn" delay={0.25}>
            <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-extrabold leading-tight max-w-5xl gradient-text">
              Smart, Reliable Infrastructure for Buildings That Never Stop Working
            </h1>
          </Reveal>

          <Reveal variant="fadeUp" delay={0.45}>
            <p className="mt-8 text-white/65 max-w-2xl text-lg md:text-xl leading-relaxed">
          ...
          <Reveal variant="fadeUp" delay={0.6}>
            <div className="mt-10 flex flex-wrap gap-4 justify-center">
              <a
                ...
                className="btn-shimmer bg-accent text-white font-semibold px-8 py-4 rounded-xl hover:bg-blue-600 transition-all duration-300 hover:shadow-[0_0_30px_rgba(37,99,235,0.4)] text-base"
              >
                Explore Services
              </a>
              <Link
                to="/contact"
                className="border border-white/25 text-white font-semibold px-8 py-4 rounded-xl hover:bg-white/10 hover:border-white/40 transition-all duration-300 text-base"
              >
                Get a Quote
              </Link>
            </div>
      ...
      <section className="relative bg-white border-b border-line">
        ...
        <div className="max-w-container mx-auto px-6 py-16 grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
          {[
            ...
            return (
              <Reveal key={stat.label} delay={i * 0.1} variant="scaleUp">
                <div className="flex flex-col items-center gap-3">
                  ...
                  <div className="text-3xl md:text-5xl font-extrabold text-navy stat-accent pb-4">
                    <AnimatedCounter value={stat.value} />
                  </div>
                  <div className="text-sm text-ink/55 font-medium mt-1">{stat.label}</div>
                </div>
              </Reveal>
            )
          })}
          ...
          <div className="grid md:grid-cols-2 gap-14 items-center">
            <Reveal variant="slideLeft">
              <div className="relative">
                ...
                <div className="absolute -bottom-4 -right-4 w-full h-full rounded-2xl bg-accent/10 border border-accent/20 z-0" />
              </div>
            </Reveal>
      ...
      {/* ═══════════════════════════════ SERVICES GRID ═══════════════════════════════ */}
      <section id="services-preview" className="bg-white py-28 scroll-mt-20">
        <div className="max-w-container mx-auto px-6">
          <Reveal>
            <div className="text-center mb-16">
          ...
          <div className="grid sm:grid-cols-2 md:grid-cols-4 gap-8">
            {team.slice(0, 4).map((member, i) => (
              <Reveal key={member.name} delay={i * 0.08} variant="scaleUp">
                <motion.div
                  className="bg-white border border-line rounded-2xl p-8 text-center card-hover-glow"
                  whileHover={{ y: -5 }}
                  transition={{ type: 'spring', stiffness: 300, damping: 20 }}
                >
                  <div className="w-20 h-20 rounded-full bg-gradient-to-br from-navy to-accent text-white flex items-center justify-center font-bold text-xl mx-auto mb-5 shadow-lg">
                  ...
                  <p className="text-accent text-xs font-semibold mt-1.5 tracking-wide uppercase">{member.role}</p>
                  <p className="text-ink/50 text-xs mt-4 leading-relaxed">{member.experience}</p>
                </motion.div>
              </Reveal>
            ))}
          </div>
      ...
      <section className="relative overflow-hidden py-28">
        ...
        {/* Accent glow */}
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[300px] bg-accent/10 rounded-full blur-[80px] animate-pulse-glow" />
        ...
        <div className="relative z-10 max-w-container mx-auto px-6 text-center">
          ...
          <Reveal variant="scaleUp" delay={0.3}>
            <Link
              to="/contact"
              className="btn-shimmer inline-block bg-accent text-white font-semibold px-10 py-4 rounded-xl hover:bg-blue-600 transition-all duration-300 hover:shadow-[0_0_40px_rgba(37,99,235,0.35)] text-lg"
            >
              Contact Us
            </Link>
          </Reveal>
